...
namespace VisiLibity
{
  ...
  Line_Segment intersection_infinite(const Line_Segment& line_segment1, const Line_Segment& line_segment2, double epsilon)
    {
        ...
        if( fabs(denom) < 1e-12 || fabs(denom) < fabs(epsilon) ) return line_segment_temp;
               ...
               double epsilon) : Point(pointemp)
  {
    ...
    and pointemp==pointemp
    and distance(polar_origin_, pointemp) <= epsilon ){
      bearing_ = Angle(0.0);
      range_ = 0.0;
    }
                  ...
                  int fios_precision_temp)
  {
    ...
    {
        if(polygonList.empty()) return true;
    ...
    bool Polygon::offset_polygons(std::vector<Polygon>& polygonList, std::vector<Polygon>& resultingPolygons, std::vector<double>& delta, double epsilon)
    {
        ...
        if(polygonList.empty()) return true;
        if(polygonList.size() != delta.size()) return false;
        int deltaValue = 0;
        if(delta[0] < -epsilon) deltaValue = -1;
        if(delta[0] > epsilon)  deltaValue =  1;
        for(n=1; n<delta.size(); n++)
        {
            int nValue = 0;
            if(delta[n] < -epsilon) nValue = -1;
            if(delta[n] > epsilon)  nValue =  1;
                     ...
                     double epsilon) const
  {
    ...
    return(  
     ...
     //and the pike is sufficiently sharp,
       and distance( Ray(observer, point1), point2 )
       <= epsilon
       and distance( Ray(observer, point3), point2 )
       <= epsilon  
         );
    //Formerly used
